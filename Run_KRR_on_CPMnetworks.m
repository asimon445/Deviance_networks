...
for t = 1:length(devpredictions)
    ...
    if isempty(israw)
        ...
        outfile = sprintf('%s/%s_CPM_network_KRRpreds.mat',outdir,header{1,devpredictions(t)});
        ...
        if ~exist(outfile,'file')
            ...
            % structure data for predictions
            ixx=0;
            for i = 1:N
            ...
            for np = 1:nperms
                ...
                CPM_train = main(train_x,train_y,kfolds,'CPM network');

                all_pos_edges(:,1:kfolds) = CPM_train.all_pos_edges;
                all_neg_edges(:,1:kfolds) = CPM_train.all_neg_edges;
                ...
                clear train_x train_y indices KRR test_x test_y pos_edge_ix neg_edge_ix edges CPM_train
            end
            fprintf('\n');
